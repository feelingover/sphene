...
会話コンテキスト維持、Google Gen AI SDK (Vertex AI) 対話、タイムアウト(30分)、最大10ターン、画像処理、ツール呼び出し制御、エラーハンドリング。
...
## Message Flow
ユーザー → Discord → イベントハンドラ(トリガー判定) → `asyncio.to_thread(process_conversation)` → Sphene.input_message → Vertex AI (Native SDK) → Discord応答
