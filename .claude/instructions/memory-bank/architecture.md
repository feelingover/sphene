...
Discord接続、コマンド/イベントハンドラ初期化、システムプロンプト読み込み、15分ごとのクリーンアップタスク（バッファ清掃・要約・プロファイル永続化・ファクトストアクリーンアップ・反省会トリガー）。
...
自律応答: `_try_autonomous_response()`, `_process_autonomous_response()`。
...
| **長期** | `fact_store.py` | ファクトストア（Fact dataclass, Jaccard×decay検索, local/Firestore永続化, 忘却機能） |
| **思考** | `reflection.py` | 反省会エンジン（LLMによるファクト抽出, fire-and-forget非同期） |
| **判定** | `judge.py` | RuleBasedJudge（スコアリング + response_type + should_react決定） |
| **判定** | `llm_judge.py` | LLMJudge（中間スコアの二次判定、絵文字選択、4-tuple返却） |

## Design Patterns

- **Per-channel instances**: `channel_conversations: defaultdict[str, Sphene]` でチャンネルごとに独立した会話（ユーザー間での履歴共有）
- **Storage abstraction**: `STORAGE_TYPE`（`local` | `firestore`）1変数でバックエンドを統一切り替え（チャンネル設定・チャンネルコンテキスト・ユーザープロファイル・ファクトストア）
- **Singleton stores**: `get_channel_context_store()`, `get_user_profile_store()`, `get_fact_store()` でインメモリキャッシュを共有
...
- **Group flags**: `VANGUARD_ENABLED`（自律応答系）/ `LIVING_MEMORY_ENABLED`（記憶系）の2フラグでデフォルト有効化
...
system_prompt
  ...
  + channel_summary  (LIVING_MEMORY_ENABLED: ローリング要約)
  + user_profile     (LIVING_MEMORY_ENABLED: 関係性・直近話題)
  + relevant_facts   (LIVING_MEMORY_ENABLED: 関連する過去の記憶)
...
ユーザー → Discord → `_handle_message()`
  ...
  → バッファ量ベース反省会トリガー（`LIVING_MEMORY_ENABLED`: メッセージ数 >= MAX_BUFFER_MESSAGES）
  → トリガー判定 (is_mentioned)
    → Yes: プロファイル取得 → `process_conversation()` → `Sphene.input_message()` → last_topic更新
    → No:  Judge評価（`VANGUARD_ENABLED`）→ `_dispatch_response()` → react / short_ack / `_process_autonomous_response()`
