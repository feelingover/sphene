...
- 記憶機能 Phase 3A（反省会 + ファクトストア）実装済み。自発的会話機能は issue #104 対応で削除。
...
- 記憶機能（Phase 1 + Phase 2 + Phase 2A + Phase 2B + Phase 3A + Phase 3B）実装済み。
...
マルチテナント競合・トリガー設計の根本見直しのため、`PROACTIVE_CONVERSATION_ENABLED` ベースの自発会話機能を一旦削除。

- 削除: `config.py`（`PROACTIVE_CONVERSATION_ENABLED`/`PROACTIVE_SILENCE_MINUTES`）、`bot/events.py`（`_try_proactive_conversation`/`_dispatch_proactive_message`/`pre_add_last_time`）、`ai/conversation.py`（`generate_proactive_message`）
- issue #104 は OPEN のまま、タイマーベースへの再設計を継続検討
...
- **修正**: `max_output_tokens=1000` に引き上げ（thinking トークン分を考慮したモデル非依存な値）。プロンプトも強化。
- **変更ファイル**: `ai/conversation.py`（`generate_short_ack()` の `GenerateContentConfig` と instruction 文字列）
...
### 2026/2: Phase 3A - 反省会 + ファクトストア

会話の長期記憶をキーワードベースで実現。チャンネルが沈黙後に反省会エンジンでファクトを抽出し、次の会話に活用する。
...
- **`bot/events.py`**:
  ...
  - `_handle_message()`: バッファ量ベース反省会トリガー追加
