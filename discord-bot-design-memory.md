...
5. **Action:** ✅ 実装済み（Phase 2A で応答多様性を追加予定）
   * スコアが閾値を超えた場合 → 応答タイプを決定し、タイプに応じた処理を実行。
   * **応答タイプ判定 (Phase 2A で追加予定):**
     * `react_only`: 絵文字リアクションのみ（同意・笑い等）。スコアが低〜中程度で、発言内容が感情的な場合。
     * `short_ack`: 一言の相槌（「わかる」「いいね」等）。スコアが中程度で、積極的に参加するほどではない場合。
     * `full_response`: 通常の応答。スコアが高い or 質問・名前呼びなど明確なトリガーがある場合。
...
6. **会話の減衰検知 (Phase 2A で追加予定):**
   * エンゲージメント期間中でも、相手の応答が短くなってきた場合（直近3件の平均文字数が減少傾向）にスコアを減衰。
...
### Phase 2A: チャンネルコンテキスト + 応答多様性 + Judge拡張
