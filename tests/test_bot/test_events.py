...
class TestDetectNickname:
    """_detect_nickname のテスト"""

    def test_match_tte_yonde(self):
        """〇〇って呼んで のパターン"""
        from bot.events import _detect_nickname
        assert _detect_nickname("ねえ、ポチって呼んでね") == "ポチ"

    def test_match_to_yondekudasai(self):
        """〇〇と呼んでください のパターン"""
        from bot.events import _detect_nickname
        assert _detect_nickname("ボブと呼んでください") == "ボブ"

    def test_match_to_yondekure(self):
        """〇〇と呼んでくれ のパターン"""
        from bot.events import _detect_nickname
        assert _detect_nickname("田中と呼んでくれ") == "田中"

    def test_match_to_yondene(self):
        """〇〇と呼んでね のパターン"""
        from bot.events import _detect_nickname
        assert _detect_nickname("次郎と呼んでね") == "次郎"

    def test_match_watashi_no_koto(self):
        """私のことを〇〇って呼んで のパターン"""
        from bot.events import _detect_nickname
        assert _detect_nickname("私のことをマリアって呼んで") == "マリア"

    def test_no_match_normal_sentence(self):
        """普通の文章はマッチしない"""
        from bot.events import _detect_nickname
        assert _detect_nickname("今日はいい天気ですね") is None

    def test_no_match_keyword_absent(self):
        """呼んでというキーワードがない場合はマッチしない"""
        from bot.events import _detect_nickname
        assert _detect_nickname("ポチという名前がいいと思う") is None

    def test_too_long_nickname_excluded(self):
        """21文字以上のニックネームは除外される"""
        from bot.events import _detect_nickname
        long_name = "あ" * 21
        assert _detect_nickname(f"{long_name}って呼んで") is None

    def test_max_length_nickname_allowed(self):
        """20文字のニックネームは許可される"""
        from bot.events import _detect_nickname
        name_20 = "あ" * 20
        assert _detect_nickname(f"{name_20}って呼んで") == name_20

    def test_empty_string(self):
        """空文字はNoneを返す"""
        from bot.events import _detect_nickname
        assert _detect_nickname("") is None
