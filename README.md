...
- **長期記憶（反省会 + ファクトストア）**: 会話が沈静化した後にLLMで事実を抽出・保存し、次の会話に活用（`LIVING_MEMORY_ENABLED`）

## セットアップ方法

### 環境変数の設定

`.env.sample` をもとに `.env` ファイルを作成し、必要な情報を入力します。各変数の説明は `.env.sample` のコメントを参照してください。
...
VANGUARD（自律応答）と Living Memory（記憶システム）はデフォルト有効です。ボットは明示的に呼びかけられなくても、会話の流れに応じて自然に参加します。

- **ルールベース判定**: キーワード一致、エンゲージメント状態、クールダウン、会話フロー分析によるスコアリング
- **LLM二次判定**: 中間スコアのメッセージに対して、LLMが会話文脈を考慮して追加判定
- **応答多様性**: スコアに応じた3段階の応答タイプ
  - **リアクション**: 低スコア時に絵文字リアクションで軽く参加（`REACTION_ENABLED` で制御）
  - **相槌**: 中スコア時に一言の短い相槌で反応
  - **フル応答**: 高スコア時にチャンネルの文脈を踏まえた本格的な応答
- **チャンネルコンテキスト**: ローリング要約でチャンネルの雰囲気・話題・参加者を把握し、応答品質を向上
- 詳細は `docs/vanguard.md` を参照
...
├── docs/                   # ドキュメント
│   ├── vanguard.md                 # VANGUARD 自律応答機能の仕様
│   └── living-memory.md            # リビングメモリー記憶システムの仕様
├── tests/                  # テストコード
│   ├── conftest.py         # テスト設定
│   ├── test_ai/            # AI機能テスト
│   ├── test_bot/           # ボット機能テスト
...
**VANGUARD（自律応答）** `VANGUARD_ENABLED=true`

- ルールベースのスコアリングによる自律応答判定
- LLMによる二次判定（曖昧スコア帯のみ呼び出し）
- エンゲージメント追跡とクールダウン制御
- キーワードベースのスコアブースト
- 応答多様性（リアクション / 相槌 / フル応答の3段階）
- 絵文字リアクション先行実行（asyncio.create_task）
- 会話フロー分析（2人会話検出、高頻度検出、沈黙後検出、会話減衰検出）

**リビングメモリー（記憶システム）** `LIVING_MEMORY_ENABLED=true`

- チャンネルメッセージバッファ（短期記憶・dequeリングバッファ）
...
- ファクトストア（Jaccard×指数減衰スコア、ベクトル検索対応、忘却機能）
